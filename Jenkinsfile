node {
    ...
    stage('SonarQube analysis') {    
          ...
          def projectKey = "android-sdk"
          def sources = "qualaroo-sdk/src/main/java"
          def binaries = "qualaroo-sdk/build/intermediates/classes/staging/debug"
          def junitReportPath = "qualaroo-sdk/build/test-results/testStagingDebugUnitTest"
          def jacocoReportPath = "qualaroo-sdk/build/jacoco/testStagingDebugUnitTest.exec"
          withSonarQubeEnv('QualarooSonarQube') {
             ...
             if (env.BRANCH_NAME == "dev") {            
                ...
                sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName='Android SDK' -Dsonar.projectKey=${projectKey} -Dsonar.sources=${sources} -Dsonar.language=java -Dsonar.java.binaries=${binaries} -Dsonar.junit.reportPaths=${junitReportPath} -Dsonar.jacoco.reportPaths=${jacocoReportPath} -Dsonar.java.source=1.7"
             } else if (env.BRANCH_NAME != "master") {
                echo "Running preview analysis because it's probably a PR" 
                sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName='Android SDK' -Dsonar.projectKey=${projectKey} -Dsonar.sources=${sources} -Dsonar.language=java -Dsonar.java.binaries=${binaries} -Dsonar.junit.reportPaths=${junitReportPath} -Dsonar.jacoco.reportPaths=${jacocoReportPath} -Dsonar.java.source=1.7 -Dsonar.github.pullRequest=${env.CHANGE_ID} -Dsonar.github.repository=qualaroo/android-mobile-sdk-src -Dsonar.github.oauth=$GITHUB_ACCESS_TOKEN -Dsonar.analysis.mode=preview -Dsonar.github.disableInlineComments=true"
             }             
          }
    }
}
